...
module.exports = {
	EmptyData: React.createClass({
		...
		render: function(){
			return (
				<div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', minHeight: 60, color: '#928d8d' }}>
					<svg style={{fontSize: 24}} aria-hidden="true" focusable="false" data-prefix="fas" data-icon="inbox" class="svg-inline--fa fa-inbox fa-w-18 " role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512">
					...
					<div>{this.props.label || 'No Data'}</div>
				</div>
			);
		}
	}),
		...
		render: function(){
			return (
				<div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', minHeight: 60, color: '#928d8d' }}>
					...
					<div>{this.props.label || 'Loading ... '}</div>
				</div>
			);
		}
	})
