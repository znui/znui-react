...
var Observable = zn.Class({
  ...
  methods: {
    ...
    watch: function watch(path, handler, context) {
      var _paths = path === '*' ? this.constructor._properties_ : zn.is(path, 'array') ? path : [path];
    ...
    unwatch: function unwatch(path, handler, context) {
      var _paths = path === '*' ? this.constructor._properties_ : zn.is(path, 'array') ? path : [path];
    ...
    notify: function notify(name, data) {
      ...
      zn.each(_names, function (_name) {
        this.__notify(_name, data);
      }, this);
      return this;
    },
    __watch: function __watch(path, handler, context) {
    ...
    __unwatch: function __unwatch(path, handler, context) {
      var _index = path.indexOf('.'),
          _name = path,
          _subPath = '',
          __watchers__ = this.__watchers__;
    ...
    __bind: function __bind(name, value) {
    ...
    __unbind: function __unbind(name, value) {
    ...
    __notify: function __notify(name, data) {
      var _value = this.get(name);

      zn.each(this.__watchers__[name], function (watcher) {
        if (watcher && watcher.handler) {
          watcher.handler.call(watcher.context, zn.path(_value, watcher.subPath), watcher.fullPath, this, data);
        }
      }, this);
    }
  }
